...
function guiControls() {
    ...
    controlFolder.add(animations, 'labels').name('Show/Hide Labels');
    animationsFolder.add(animations, 'image').name('Show/Hide Diagram');
...
function rayAction() {
    if (iRay) {
        ...
        new TWEEN.Tween(camera.position)
            ...
            .easing(TWEEN.Easing.Cubic.Out)
            .start()
        createCamera(30, -45, 10, 60, new THREE.Vector3(-10, 18, 0));
        createControls(-10, 18, -4.5);
        for (let i=0; i<rayObjects.length; i++) {
            rayObjects[i].visible = true;
        }
        partEl.innerHTML = 'Ray Diagram';
        describeEl.innerHTML = '1 - Object (Specimen); 2 - Objective Lens; 3 -  Eyepiece Lens;    4 - Intermediate Image; 5 - Final Inverted Image'; 
        rayDiagram ()
    }else {
